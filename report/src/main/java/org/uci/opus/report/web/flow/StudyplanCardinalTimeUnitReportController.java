 ...
 * in which case the provisions of the GPL or the LGPL are applicable insteads
...
@Controller
@RequestMapping(value = "/report/studyplancardinaltimeunitreport.view")
@SessionAttributes({"reportForm"})
public class StudyplanCardinalTimeUnitReportController extends OpusReportsController2 {

    private static Logger log = LoggerFactory.getLogger(StudyplanCardinalTimeUnitReportController.class);
    ...
    public StudyplanCardinalTimeUnitReportController() {
        super();
        viewName = "/report/report/studyPlanCardinalTimeUnitReport";
    }
    ...
    public ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String reportName = ServletUtil.getParamSetAttrAsString(request, "reportName", "StudentTimeUnitDeclaration");
       
        /*ServletUtil.getParamSetAttrAsString(request, "orderBy", "person_surnamefull");
        String searchValue = ServletUtil.getParamSetAttrAsString(request, "searchValue", "").trim();
        ServletUtil.getParamSetAttrAsString(request, "titleKey", "");
        ServletUtil.getParamSetAttrAsBoolean(request, "multiSelect", true);

        parameterMap.put("searchValue", searchValue.trim());
        request.setAttribute("action", "/report/studyplancardinaltimeunitreport.view");
        ...
        */
        ...
        mav.addObject("action", "/report/studyplancardinaltimeunitreport.view");
        ...
        if ((ReportForm) session.getAttribute("reportForm") != null) {
            ...
            String reportDir = session.getServletContext().getRealPath("/WEB-INF/reports/jasper/");

            String reportPath = reportDir + System.getProperty("file.separator") + reportName + ".jasper";
        ...
        if (StringUtil.isNullOrEmpty(operation, true)) {
            ...
            List<Map<String, Object>> items = this.reportMapper.findStudyplanCardinalTimeUnits(parameterMap);
            ...
            String downloadFileName = ServletUtil.getParamSetAttrAsString(request, "downloadFileName",
                    messageSource.getMessage("report.studentcertificate.title", null, locale));
            // spaces cause the name to be incomplete
            downloadFileName = downloadFileName.replaceAll("\\s+", "_");

            mav = reportController.createReport(session, reportName, requestWhereClause, downloadFileName, reportFormat, locale);
           ...
           // Map<String, Object> allProperties = getUserProperties("studyplancardinaltimeunitreport");
            Map allProperties = getUserProperties("studyplancardinaltimeunitreport");

          //get global properties from database
           // Map allProperties = getUserProperties(reportForm.getReportName().toLowerCase());
    ...
    public ModelAndView processSubmit(@ModelAttribute("reportForm") ReportForm reportForm, BindingResult result,
            HttpServletRequest request) throws JRException {

        ModelAndView mav = new ModelAndView("redirect:/report/studyplancardinaltimeunitreport.view");
		...
		if ("makereport".equalsIgnoreCase(operation)) {
	           ...
	           String downloadFileName = ServletUtil.getParamSetAttrAsString(request, "downloadFileName"
							, messageSource.getMessage("report.StudentTimeUnitDeclaration.title", null, locale));
	           //spaces cause the name to be incomplete
	           downloadFileName = downloadFileName.replaceAll("\\s+", "_");
