...
@Controller
@RequestMapping(value = "/report/studentdeclaration.view")
@SessionAttributes({ "reportForm" })
public class StudentDeclarationReportController extends OpusReportsController2 {

	private static Logger log = LoggerFactory.getLogger(StudentDeclarationReportController.class);
	...
	// TODO refactor this controller Should use same controller as
	// studentprofile sheet
	public StudentDeclarationReportController() {
		super();
		viewName = "/report/report/studentdeclaration";
	}
	...
	public ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		Map<String, Object> parameterMap = this.loadAndMakeParameterMap(fb,
				request, session, null);
		ModelAndView mav = new ModelAndView(viewName);

		String reportFormat = ServletUtil.getParamSetAttrAsString(request,
				"reportFormat", "pdf");
		String reportName = ServletUtil.getParamSetAttrAsString(request,
				"reportName", "StudentDeclaration");
		String orderBy = ServletUtil.getParamSetAttrAsString(request,
				"orderBy", "person_surnamefull");
		...
		mav.addObject("action", "/report/studentdeclaration.view");
		mav.addObject("titleKey", request.getParameter("titleKey"));

		ReportForm reportForm = null;

		opusMethods.removeSessionFormObject("reportForm", session,
		...
		if ((ReportForm) session.getAttribute("reportForm") != null) {
			 ...
			 //Termina a sessao e retira os parametros 
			/*reportForm = null;
			...
			reportForm.setProperties(ReportControllerUtil
					.getReportPropertiesFromFile(reportPath, reportName,
		...
		if (StringUtil.isNullOrEmpty(operation, true)) {
			...
			// this.studentReportManager.findStudentsByName(parameterMap);
			List<Map<String, Object>> students = this.reportMapper
					.findStudyplanCardinalTimeUnits(parameterMap);

			mav.addObject("students", students);

			mav.addObject("MAX_DISPLAY_ITEMS",
					ReportConstants.MAX_DISPLAY_ITEMS);

			// instructs the page to show all items regardless of the number of
			// items exceed the MAX_DISPLAY_ITEMS
			ServletUtil.getParamSetAttrAsString(request, "showAll", "false");

		} else if ("makereport".equalsIgnoreCase(operation)) {

			String requestWhereClause = reportController
					.generateSQLWhereClause(request);

			// at least one student needs to be selected
			if (requestWhereClause == null
					|| requestWhereClause.trim().isEmpty()) {
				// avoid displaying all results but show empty report
				requestWhereClause = " AND false ";
			}

			String downloadFileName = ServletUtil.getParamSetAttrAsString(
					request, "downloadFileName", messageSource.getMessage(
							"report.studentdeclaration.title", null, locale));
			...
			mav = reportController.createReport(session, reportName,
					requestWhereClause, downloadFileName, reportFormat, locale);

			StringBuilder whereClause = new StringBuilder();

			if (parameterMap.containsKey("organizationalUnitId")) {
				int organizationalUnitId = (Integer) parameterMap
						.get("organizationalUnitId");

				whereClause
						.append(" AND organizationalunit.id in (SELECT id FROM opuscollege.crawl_tree(");
				whereClause.append(organizationalUnitId).append(", 0))");
			}

			// get global properties from database
			Map allProperties = getUserProperties("studentdeclaration");
	...
	public ModelAndView processSubmit(
		...
		ModelAndView mav = new ModelAndView(
				"redirect:/report/studentdeclaration.view");
	...
	private Map<String, Object> getProperties(List<ReportProperty> properties) {
		...
		for (ReportProperty property : properties) {
			...
			// ignore reportLogo and reportBackrounf as they are universal
			// properties
			if ((!property.getName().equalsIgnoreCase("reportLogo"))
					&& (!property.getName()
							.equalsIgnoreCase("reportBackground")))
