...
<c:set var="screentitlekey">jsp.lookuptable.${lookup3Form.lookupTable}.label1</c:set>
...
<div id="tabwrapper"><%@ include file="../menu.jsp"%>
    ...
    <spring:bind path="lookup3Form.navigationSettings">
...
<a href="<c:url value='/college/lookups.view?lookupTable=${lookup3Form.lookupTable}&amp;operation=edit&amp;language=${primaryLookup.lang}&amp;lookupCode=${primaryLookup.code}&amp;currentPageNumber=${lookup3Form.navigationSettings.currentPageNumber}&amp;institutionTypeCode=${institutionTypeCode}'/>">
    <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup3Form.lookupTable)}.label2" /> </a> &nbsp; > &nbsp;    
     <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup3Form.lookupTable)}.label1" />/
     
     <c:choose>
        <c:when test="${not empty lookup3Form.lookupCode}">
...
<form:form method="post" commandName="lookup3Form">
        
    <c:forEach items="${lookup3Form.lookups}" varStatus="row" var="lookup">
		<form:hidden path="lookups[${row.index}].lang"/>
    	<form:hidden path="lookups[${row.index}].id"/>
    	<form:hidden path="lookups[${row.index}].code"/>
    	<form:hidden path="lookups[${row.index}].short2"/>                
    	<form:hidden path="lookups[${row.index}].short3"/>

		<%--only edit lookups for language currently installed --%>
        <c:set var="containsLanguage" value="false" />
        <c:forEach var="item" items="${lookup3Form.appLanguagesShort}">
    	...
    	<c:choose>
        	...
        	<fieldset>
        		<legend>
        			<fmt:message key="jsp.language.${fn:trim(lookup3Form.lookups[row.index].lang)}"/>
        		</legend>        
	...
	<tr>
            <td>
                <label>
                    <fmt:message key="jsp.general.lookup.short2"/>
                </label>
             </td>
             <td>
                <form:input path="lookupShort2" maxlength="2" size="50"/>
                (<fmt:message key="jsp.general.maxcharacters.args">
                        <fmt:param>2</fmt:param>
                    </fmt:message>)
             </td>
             <form:errors path="lookupShort2" cssClass="error" element="td"/>            
        </tr>
	<tr>
            <td>
                <label>
                    <fmt:message key="jsp.general.lookup.short3"/>
                </label>
             </td>
             <td>
                <form:input path="lookupShort3" maxlength="3" size="50"/>
                (<fmt:message key="jsp.general.maxcharacters.args">
                        <fmt:param>3</fmt:param>
                    </fmt:message>)
             </td>
             <form:errors path="lookupShort3" cssClass="error" element="td"/>            
        </tr>
		<tr>
		<td>
			<label>
