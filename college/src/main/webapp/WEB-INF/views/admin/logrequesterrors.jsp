...
<fieldset>
    <legend>
        <fmt:message key="jsp.logrequesterrors.header" />
    </legend>
    ...
    <form name="reportForm" id="reportForm" method="get" target="_blank" action="<c:url value='/college/reports.view'/>">
 		...
 		<table style="padding:20px 0px 10px  0px" >
                    ...
                    <select name="reportName">                    
                    	<option value="LogRequestErrors"z>
                    		<fmt:message key="jsp.general.list"/>
                    	</option>
                    	<option value="LogRequestError" <c:if test="${reportName == 'LogRequestError'}"> selected="selected"</c:if>>
		...
		<c:set var="allEntities" value="${logRequestErrors}" scope="page" />
		<c:set var="redirView" value="logrequesterrors" scope="page" />
        ...
        <table class="tabledata" id="TblData">
        	...
        	<th><input type="checkbox" name="checker" id="checker" checked="checked" onclick="javascript:toggleGroup('where.id', this.checked);"/></th>
            <th><fmt:message key="jsp.general.id" /></th>
            <th><fmt:message key="jsp.general.ipaddress" /></th>
            <th><fmt:message key="jsp.general.requeststring" /></th>                  
            <th><fmt:message key="jsp.general.date" /></th>
            <th><fmt:message key="jsp.general.message" /></th>
            <th>&nbsp;</th>
            
            <c:forEach var="logRequestError" items="${logRequestErrors}">
            	...
            	<c:choose>
            		<c:when test="${(entityNumber < (currentPageNumber*initParam.iPaging) + 1) && (entityNumber > ((currentPageNumber*initParam.iPaging) - initParam.iPaging)) }" >
		                <tr>
		                <td align="center"><input type="checkbox" name="where.id" checked="checked"  value="${logRequestError.id}"></td>
		                <td>${logRequestError.id}</td>
		                <td>${logRequestError.ipAddress}</td>
		                <td>
		                <c:choose>
                          <%-- show only the 30 first characters, so it doesn't take to much space on screen --%>
                          <c:when test="${fn:length(logRequestError.requestString) > 30}">
                          	${fn:substring(fn:escapeXml(logRequestError.requestString), 0, 30)}...
                           </c:when>
                           	<c:otherwise>
                           		${fn:escapeXml(logRequestError.requestString)}                      			
                            </c:otherwise>
                          </c:choose>
		                </td>
		                </td>	                
		                <td style="text-align: center"><fmt:formatDate value="${logRequestError.date}" dateStyle="SHORT"/></td>
		                <td>
		                <c:choose>
                          <%-- show only the 30 first characters, so it doesn't take to much space on screen --%>
                          <c:when test="${fn:length(logRequestError.errorMessage) > 30}">
                          	${fn:substring(fn:escapeXml(logRequestError.errorMessage), 0, 30)}...
                           </c:when>
                           	<c:otherwise>
                           		${fn:escapeXml(logRequestError.errorMessage)}                      			
                            </c:otherwise>
                          </c:choose>
		                </td>
		                
		                <td style="text-align:center;text-decoration: none">
		                <a href="<c:url value='/college/reports.view'/>?reportName=LogRequestError&reportFormat=pdf&where.id=${logRequestError.id}"
		                target="_blank"
                    ...
                    |
                    <a href="<c:url value='/college/reports.view'/>?reportName=LogRequestError&reportFormat=xls&where.id=${logRequestError.id}"
                    target="_blank"
