...
<c:set var="screentitlekey">jsp.lookuptable.${lookup8Form.lookupTable}.label1</c:set>
...
<div id="tabwrapper"><%@ include file="../menu.jsp"%>
    ...
    <spring:bind path="lookup8Form.navigationSettings">
...
<a href="<c:url value='/college/lookups.view?lookupTable=${lookup8Form.lookupTable}&amp;operation=edit&amp;language=${primaryLookup.lang}&amp;lookupCode=${primaryLookup.code}&amp;currentPageNumber=${lookup8Form.navigationSettings.currentPageNumber}&amp;institutionTypeCode=${institutionTypeCode}'/>">
    <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup8Form.lookupTable)}.label2" /> </a> &nbsp; > &nbsp;    
     <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup8Form.lookupTable)}.label1" />/
     
     <c:choose>
        <c:when test="${not empty lookup8Form.lookupCode}">
...
<form:form method="post" commandName="lookup8Form">

    <c:forEach items="${lookup8Form.lookups}" varStatus="row" var="lookup">
        
        <form:hidden path="lookups[${row.index}].lang"/>
	    <form:hidden path="lookups[${row.index}].id"/>
    	<form:hidden path="lookups[${row.index}].code"/>
    	<form:hidden path="lookups[${row.index}].nrOfUnitsPerYear"/>

        <%--only edit lookups for language currently installed --%>
        <c:set var="containsLanguage" value="false" />
        <c:forEach var="item" items="${lookup8Form.appLanguagesShort}">
        ...
        <c:choose>
        	<c:when test="${containsLanguage }">	
            	<fieldset>
            		<legend>
            			<fmt:message key="jsp.language.${fn:trim(lookup8Form.lookups[row.index].lang)}"/>
            		</legend>        
     <table>
        	<tr>
        		<td>
	...
	<tr>
            <td>
                <label>
                    <fmt:message key="jsp.general.numberofunitsperyear"/>
                </label>
             </td>
             <td>
                <form:input path="lookupNrOfUnitsPerYear" size="50" cssClass="numericField"/>
                
             </td>
             <form:errors path="lookupNrOfUnitsPerYear" cssClass="error" element="td"/>            
        </tr>
		<tr>
		<td>
			<label>
