...
<c:set var="screentitlekey">jsp.lookuptable.${lookup5Form.lookupTable}.label1</c:set>
...
<div id="tabwrapper"><%@ include file="../menu.jsp"%>
    ...
    <spring:bind path="lookup5Form.navigationSettings">
...
<a href="<c:url value='/college/lookups.view?lookupTable=${lookup5Form.lookupTable}&amp;operation=edit&amp;language=${primaryLookup.lang}&amp;lookupCode=${primaryLookup.code}&amp;currentPageNumber=${lookup5Form.navigationSettings.currentPageNumber}&amp;institutionTypeCode=${institutionTypeCode}'/>">
    <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup5Form.lookupTable)}.label2" /> </a> &nbsp; > &nbsp;    
     <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup5Form.lookupTable)}.label1" />/
     
     <c:choose>
        <c:when test="${not empty lookup5Form.lookupCode}">
...
<form:form method="post" commandName="lookup5Form">
        
    <c:forEach items="${lookup5Form.lookups}" varStatus="row" var="lookup">
    	
    	<form:hidden path="lookups[${row.index}].lang"/>
    	<form:hidden path="lookups[${row.index}].id"/>
    	<form:hidden path="lookups[${row.index}].code"/>
    	<form:hidden path="lookups[${row.index}].countryCode"/>  
    	
    	<%--only edit lookups for language currently installed --%>
        <c:set var="containsLanguage" value="false" />
        <c:forEach var="item" items="${lookup5Form.appLanguagesShort}">
        ...
        <c:choose>
        	<c:when test="${containsLanguage }">
            	<fieldset>
            		<legend>
            			<fmt:message key="jsp.language.${fn:trim(lookup5Form.lookups[row.index].lang)}"/>
            		</legend>        
	...
	<table>
	 <tr>
            <td>
                <label>
                    <fmt:message key="jsp.general.country"/>
                </label>
             </td>
             <td>
                <form:select path="lookupCountryCode" >
                    <form:option value=""><fmt:message key="jsp.selectbox.choose"/></form:option>
                    <form:options items="${lookup5Form.countries}" itemLabel="description" itemValue="code"/>
                </form:select>
             </td>
             <form:errors path="lookupCountryCode" cssClass="error" element="td"/>            
        </tr>
		<tr>
		<td>
			<label>
