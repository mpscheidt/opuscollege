...
<c:set var="screentitlekey">jsp.lookuptable.${lookup4Form.lookupTable}.label1</c:set>
...
<div id="tabwrapper"><%@ include file="../menu.jsp"%>
    ...
    <spring:bind path="lookup4Form.navigationSettings">
...
<a href="<c:url value='/college/lookups.view?lookupTable=${lookup4Form.lookupTable}&amp;operation=edit&amp;language=${primaryLookup.lang}&amp;lookupCode=${primaryLookup.code}&amp;currentPageNumber=${lookup4Form.navigationSettings.currentPageNumber}&amp;institutionTypeCode=${institutionTypeCode}'/>">
    <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup4Form.lookupTable)}.label2" /> </a> &nbsp; > &nbsp;    
     <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup4Form.lookupTable)}.label1" />/
     
     <c:choose>
        <c:when test="${not empty lookup4Form.lookupCode}">
...
<form:form method="post" commandName="lookup4Form">
        
    <c:forEach items="${lookup4Form.lookups}" varStatus="row" var="lookup">
       
       	<form:hidden path="lookups[${row.index}].lang"/>
        <form:hidden path="lookups[${row.index}].id"/>
        <form:hidden path="lookups[${row.index}].code"/>
        <form:hidden path="lookups[${row.index}].districtCode"/>
        
        <%--only edit lookups for language currently installed --%>
        <c:set var="containsLanguage" value="false" />
        <c:forEach var="item" items="${lookup4Form.appLanguagesShort}">
        ...
        <c:choose>
        	<c:when test="${containsLanguage }">
            	<fieldset>
            		<legend>
            			<fmt:message key="jsp.language.${fn:trim(lookup4Form.lookups[row.index].lang)}"/>
            		</legend>       
     <table>
        	<tr>
        		<td>
	...
	<table>
	 <tr>
            <td>
                <label>
                    <fmt:message key="jsp.general.district"/>
                </label>
             </td>
             <td>
                <form:select path="lookupDistrictCode" >
                    <form:option value=""><fmt:message key="jsp.selectbox.choose"/></form:option>
                    <form:options items="${lookup4Form.districts}" itemLabel="description" itemValue="code"/>
                </form:select>
             </td>
             <form:errors path="lookupDistrictCode" cssClass="error" element="td"/>            
        </tr>
		<tr>
		<td>
			<label>
