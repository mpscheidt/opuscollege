...
<c:set var="screentitlekey">jsp.lookuptables.header</c:set>
...
<div id="tabwrapper">
 ...
 <fieldset>
 <legend>
 <fmt:message key="jsp.lookuptables.header" />
    </legend>
                    ...
                    <td class="label"><fmt:message key="jsp.general.search" /></td>
                        <td width="700" align="left">
                           <img src="<c:url value='/images/trans.gif' />" width="10"/>            
                            <input type="text" name="searchTable" id="searchTable"  value="${searchTable}"/>&nbsp;
        ...
        <c:set var="allEntities" value="${allLookupTables}" scope="page" />
        <c:set var="redirView" value="lookuptables" scope="page" />
        ...
        <table class="tabledata" id="TblData">
            ...
            <c:forEach var="lookupTable" items="${allLookupTables}">
                ...
                <c:choose>
                    <c:when test="${(entityNumber < (currentPageNumber*initParam.iPaging) + 1) && (entityNumber > ((currentPageNumber*initParam.iPaging) - initParam.iPaging)) }" >
                        <tr>
                           <td>
                                    <a href="
                                    <c:url value='/college/lookups.view?lookupTable=${lookupTable.tableName}&lookupType=${lookupTable.lookupType}'/>">
                                    <fmt:message key="jsp.lookuptable.${lookupTable.tableName}.label2" />
                                    </a>
                                    &nbsp;
                           </td>
                           <td>
                            <fmt:message key="jsp.lookuptable.${lookupTable.tableName}.description" />
                           </td>
                        </tr>
                    </c:when>
                </c:choose>
