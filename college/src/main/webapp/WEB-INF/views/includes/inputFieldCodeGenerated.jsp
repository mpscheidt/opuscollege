...
<%@ taglib uri="http://www.springframework.org/security/tags" prefix="sec" %>
       ...
       <tr>
           ...
           <sec:authorize access="hasAnyRole('CREATE_LOOKUPS','UPDATE_LOOKUPS','CREATE_STUDENTS','UPDATE_STUDENTS','CREATE_STAFFMEMBERS','UPDATE_STAFFMEMBERS')">
                <c:set var="editData" value="${true}"/>
                <td>
                       <input type="text" name="${status.expression}" id="${status.expression}" value="<c:out value="${status.value}" />" size="40" />
                   </td>
                   <td><fmt:message key="jsp.general.message.codegenerated" />&nbsp;<c:forEach var="error" items="${status.errorMessages}"><span class="error">${error}</span></c:forEach></td>
           </sec:authorize>
           <c:if test="${not editData}">
                <sec:authorize access="hasAnyRole('READ_LOOKUPS','READ_STUDENTS','READ_STAFFMEMBERS')">
                   <td>${status.value}</td>
                   <td></td>
                </sec:authorize>
           </c:if>
