 ...
 *
 */
 public class RolePrivilegesDeleteController extends AbstractController {
    
     private static Logger log = LoggerFactory.getLogger(RolePrivilegesDeleteController.class);
     ...
     @Autowired
    public RolePrivilegesDeleteController(SecurityChecker securityChecker, OpusUserManagerInterface opusUserManager) {
		super();
    ...
    protected final ModelAndView handleRequestInternal(HttpServletRequest request, final HttpServletResponse response) {
    	...
    	opusUserManager.deletePrivilegesFromRole(role,request.getParameterValues("privilegesCodes"));
    	
        return new ModelAndView("redirect:/college/role.view?roleName=" + role); 
    }
