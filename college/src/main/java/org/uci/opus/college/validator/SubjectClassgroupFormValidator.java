...
@Component
public class SubjectClassgroupFormValidator implements Validator {

	@Autowired private StudyManagerInterface studyManager;
	
	@Override
    public boolean supports(final Class<?> clazz) {
        return SubjectClassgroupForm.class.isAssignableFrom(clazz);
    }

	@Override
    public void validate(final Object obj, final Errors errors) {
    	
    	SubjectClassgroupForm form = (SubjectClassgroupForm)obj;

    	int classgroupId = form.getClassgroupId();
    	int studyGradeTypeId = form.getStudyGradeType() != null ? form.getStudyGradeType().getId() : 0; 
    	int subjectId = form.getSubject() != null ? form.getSubject().getId() : 0;
    	
        if (classgroupId == 0) {
            errors.rejectValue("classgroupId", "invalid.empty.format");
        }
        if (studyGradeTypeId == 0) {
            errors.reject("invalid.empty.format");
        }
        if (subjectId == 0) {
            errors.reject("invalid.empty.format");
        }
        
		if (studyGradeTypeId != 0 && subjectId != 0 && classgroupId != 0) {
			...
			if (classgroups != null) {
				for (Classgroup otherClassgroup : classgroups) {
					if (otherClassgroup.getId() == classgroupId) {
						// classgroup is already in this subject  
						errors.rejectValue("classgroupId", "jsp.error.subjectclassgroup.isalreadyinsubjectclassgroup");
					}
				}
			}
		}
