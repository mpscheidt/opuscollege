...
@Controller
@RequestMapping(value = "/college/report/admissionletter.view")
public class AdmissionLetterReportController {

    private static Logger log = LoggerFactory.getLogger(AdmissionLetterReportController.class);
    ...
    public AdmissionLetterReportController() {
        super();
    }

    @RequestMapping(method = RequestMethod.GET)
    public ModelAndView handleRequestInternal(@RequestParam("studyPlanId") int studyPlanId, HttpServletRequest request) {
        ...
        String reportName = ServletUtil.getParamSetAttrAsString(request, "reportName", "AdmissionLetter");
        String reportFullName = "person/" + reportName;// full path to report

        locale = OpusMethods.getPreferredLocale(request);
        String requestWhereClause = reportController.generateSQLWhereClause(request);

        mav = reportController.createReport(session, reportFullName, requestWhereClause, reportFormat, locale);

        String whereClause = " AND studyPlan.id= " + studyPlanId;

        reportController.addWhereClause(mav, whereClause);
        reportController.putModelParameter(mav, "lang", preferredLanguage);
        reportController.putModelParameters(mav, getUserProperties("admissionletter"));
            ...
            if (!property.getType().equalsIgnoreCase("text")) {
                ...
                if (property.isVisible()) {
                    if ((property.getFile().length == 0)) {
                        // reportController.putModelParameter(mav, "reportLogo",
