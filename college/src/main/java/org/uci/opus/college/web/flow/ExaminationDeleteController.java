 ...
 */
 public class ExaminationDeleteController extends AbstractController {
    
    private static Logger log = LoggerFactory.getLogger(ExaminationDeleteController.class);
...
//        request.setAttribute("tab", tab);
        ...
        if (examinationId != 0) {
            ...
            examinationResultList = (ArrayList < ExaminationResult >)
            		resultManager.findExaminationResults(examinationId);
            if (examinationResultList.size() != 0) {
                // show error for linked examination results
                showExaminationError = messageSource.getMessage(
                		"jsp.error.examination.delete", null, currentLoc);
                showExaminationError = showExaminationError + messageSource.getMessage(
                        "jsp.error.general.delete.linked.examinationresult", null, currentLoc);
            }
            
            if ("".equals(showExaminationError)) {
	        	// examination cannot be deleted if one or more test results are present
	            List < Test > testList = null;
	            testList = (ArrayList < Test >) testManager.findTestsForExamination(examinationId);
	            
	            for (int i = 0; i < testList.size(); i++) {
		            List < TestResult > testResultList = null;
		            testResultList = (ArrayList < TestResult >)
		            		resultManager.findTestResults(testList.get(i).getId());
	            ...
	            // if no testresults present, delete all underlying tests
	            if ("".equals(showTestError)) {
	                for (int i = 0; i < testList.size(); i++) {
	                	testManager.deleteTest(testList.get(i).getId());
	                }
	            }
            }
            
            // if no testresults and examinationresults present, delete examination
            if ("".equals(showExaminationError) && "".equals(showTestError)) {
            	examinationManager.deleteExamination(examinationId);
            } else {
            	showExaminationError = showExaminationError + showTestError;
            }
        }
        
        this.setViewName("redirect:/college/subject.view?newForm=true&tab=" + tab + "&panel=" + panel 
                        + "&subjectId=" + subjectId 
