...
<c:choose>
    <c:when test="${(fn:length(students) lt MAX_DISPLAY_ITEMS) || (showAll == true)}">
        ...
        <div id="abc">&nbsp;</div>
          ...
          <tr> 
            <th><input type="checkbox" name="checker" id="checker" checked="checked" onclick="javascript:checkAll('where.studyplan.id');"/><fmt:message key='jsp.general.create.report' /></th>
            ...
            <c:forEach var="student" items="${students}">
                    <tr>
                        <td align="center"><input type="checkbox"  paramGroup="where.studyplan.id" name="where.studyplan.id" checked="checked"  value="${student.studyplanid}"></td>
                         <td>
                            ${student.studentcode}
                         </td>                        
                        ...
                        <td>
                            <c:choose>
                                ...
                                <a href="#" 
                                    class="reportLink" 
                                    target="_self"
                                    onclick='createReport("<c:url value="/report/studentdeclaration.view" />" 
                                                , "where.studyplan.id=${student.studyplanid}&downloadFileName=<fmt:message key='jsp.general.studentdeclaration'/>_${student.firstnamesfull} ${student.surnamefull}" +  buildQueryStringByClass("reportParam"));return false;'
                                    >
                       ...
                       <c:choose>
                                ...
                                <a href="#" 
                                    ...
                                    onclick='createReport("<c:url value="/report/studentdeclaration.view" />" 
                                                , "where.studyplan.id=${student.studyplanid}&downloadFileName=<fmt:message key='jsp.general.studentdeclaration'/>_${student.firstnamesfull} ${student.surnamefull}" +  buildQueryStringByClass("reportParam"));return false;'
                                    >
                                    ${student.studyplandescription}</a>
                                </c:when>
                                <c:otherwise>
                                    ${student.studyplandescription}
                                </c:otherwise>
                            </c:choose>
                       </td>
    ...
    <c:otherwise>
        ...
        <a target="_self" href="<c:url value='/report/studentdeclaration.view'/>?<%=request.getQueryString() %>&showAll=true">
