...
<%@ include file="standardincludes.jsp"%>
    ...
    <%--
        ...
        Either 'allEntities' or 'countAllEntities' required.

    --%>
    
    <c:if test="${empty countAllEntities}">
        <c:set var="countAllEntities" value="${fn:length(allEntities)}" />
    </c:if>
    ...
    <c:set var="totalNumberOfPages"><fmt:formatNumber value="${countAllEntities/initParam.iPaging}" maxFractionDigits="0" groupingUsed="${false}" /></c:set>
 	<c:choose>
 		<c:when test="${((totalNumberOfPages * initParam.iPaging) < countAllEntities) && (countAllEntities - (totalNumberOfPages * initParam.iPaging) < initParam.iPagingTurnover)}">
			<c:set var="totalNumberOfPages" value="${totalNumberOfPages +1}" />
		</c:when>
 	</c:choose>
 	<c:choose>
		<c:when test="${currentPageNumber == null || currentPageNumber == '0'}">
