...
<sec:authorize
	access="hasAnyRole('READ_PRIMARY_AND_CHILD_ORG_UNITS','UPDATE_ORG_UNITS')">
...
<table width="100%">
	...
	<tr>
		...
		<td><select name="studyGradeTypeId" id="studyGradeTypeId"
	...
	<tr>
		<td><label><fmt:message
					key="jsp.general.organizationalunit" /></label></td>
		<td><c:choose>
				<c:when test="${readUpdateOrgUnits}">
					<select name="organizationalUnitId" id="organizationalUnitId"
						onchange="
                                    document.getElementById('primaryStudyId').value='0';
                                    document.getElementById('academicYearId').value='0';
                                    document.getElementById('studyGradeTypeId').value='0';                                
						...
						<c:forEach var="oneOrganizationalUnit"
							...
							<c:choose>
								<c:when test="${(branchId == null) }">
									<c:choose>
										<c:when
											...
											<option value="${oneOrganizationalUnit.id}"
												selected="selected">${oneOrganizationalUnit.organizationalUnitDescription}</option>
										</c:when>
										<c:otherwise>
								...
								<c:otherwise>
									<c:choose>
										<c:when
											test="${(branchId == oneOrganizationalUnit.branchId) }">
											<c:choose>
												<c:when
													test="${(organizationalUnitId == null && oneOrganizationalUnit.id == organizationalUnit.id)
                                                    || (organizationalUnitId != null && oneOrganizationalUnit.id == organizationalUnitId) }">
		...
		<td><label><fmt:message key="jsp.general.status" /></label></td>
		<td><select name="statusCode" id="statusCode" requestParam="true"
			paramGroup="filter" ignoreValue="0">
				<option value="0"><fmt:message key="jsp.selectbox.choose" /></option>
	...
	<tr>
		...
		<td><select id="primaryStudyId" name="primaryStudyId"
			onchange="
                                document.getElementById('academicYearId').value='0';
                                document.getElementById('studyGradeTypeId').value='0';                                
                                document.mainForm.submit();                                
		...
		<td><select name="orderBy" id="orderBy" reportParam="true">
				<c:choose>
	...
	<tr>
		...
		<td><select name="nameFormat" id="nameFormat" reportParam="true">
				<c:choose>
					<c:when
