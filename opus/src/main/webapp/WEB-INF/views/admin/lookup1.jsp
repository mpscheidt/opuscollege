...
<c:set var="screentitlekey">jsp.lookuptable.${lookup1Form.lookupTable}.label1</c:set>
...
<div id="tabwrapper"><%@ include file="../menu.jsp"%>
    ...
    <spring:bind path="lookup1Form.navigationSettings">
...
<a href="<c:url value='/college/lookups.view?lookupTable=${lookup1Form.lookupTable}&amp;operation=edit&amp;language=${primaryLookup.lang}&amp;lookupCode=${primaryLookup.code}&amp;currentPageNumber=${lookup1Form.navigationSettings.currentPageNumber}&amp;institutionTypeCode=${institutionTypeCode}'/>">
    <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup1Form.lookupTable)}.label2" /> </a> &nbsp; > &nbsp;    
     <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup1Form.lookupTable)}.label1" />/
     
     <c:choose>
        <c:when test="${not empty lookup1Form.lookupCode}">
...
<form:form method="post" commandName="lookup1Form">
        
    <c:forEach items="${lookup1Form.lookups}" varStatus="row" var="lookup">
    
    <form:hidden path="lookups[${row.index}].lang"/>
    <form:hidden path="lookups[${row.index}].id"/>
    <form:hidden path="lookups[${row.index}].code"/>
    <form:hidden path="lookups[${row.index}].descriptionShort"/>
        
    <%--only edit lookups for language currently installed --%>
    <c:set var="containsLanguage" value="false" />
    <c:forEach var="item" items="${lookup1Form.appLanguagesShort}">
    ...
    <c:choose>
    	<c:when test="${containsLanguage }">
        	<fieldset>
        		<legend>
        			<fmt:message key="jsp.language.${fn:trim(lookup1Form.lookups[row.index].lang)}"/>
        		</legend>        
     <table>
        	<tr>
        		<td>
	...
	<tr>
            <td>
                <label>
                    <fmt:message key="jsp.general.lookup.short"/>
                </label>
             </td>
             <td>
                <form:input path="lookupDescriptionShort" maxlength="3" size="50"/>
                (<fmt:message key="jsp.general.maxcharacters.args">
                        <fmt:param>3</fmt:param>
                    </fmt:message>)
             </td>
             <form:errors path="lookupDescriptionShort" cssClass="error" element="td"/>            
        </tr>
		<tr>
		<td>
			<label>
