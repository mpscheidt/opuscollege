...
<c:set var="screentitlekey">jsp.lookuptable.${lookup6Form.lookupTable}.label1</c:set>
...
<div id="tabwrapper"><%@ include file="../menu.jsp"%>
    ...
    <spring:bind path="lookup6Form.navigationSettings">
...
<a href="<c:url value='/college/lookups.view?lookupTable=${lookup6Form.lookupTable}&amp;operation=edit&amp;language=${primaryLookup.lang}&amp;lookupCode=${primaryLookup.code}&amp;currentPageNumber=${lookup6Form.navigationSettings.currentPageNumber}&amp;institutionTypeCode=${institutionTypeCode}'/>">
    <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup6Form.lookupTable)}.label2" /> </a> &nbsp; > &nbsp;    
     <fmt:message key="jsp.lookuptable.${fn:toLowerCase(lookup6Form.lookupTable)}.label1" />/
     
     <c:choose>
        <c:when test="${not empty lookup6Form.lookupCode}">
...
<form:form method="post" commandName="lookup6Form">
        
    <c:forEach items="${lookup6Form.lookups}" varStatus="row" var="lookup">
    	...
    	<form:hidden path="lookups[${row.index}].titleShort"/>
    	
    	<%--only edit lookups for language currently installed --%>
        <c:set var="containsLanguage" value="false" />
        <c:forEach var="item" items="${lookup6Form.appLanguagesShort}">
        ...
        <c:choose>
        	<c:when test="${containsLanguage }">
            	<fieldset>
            		<legend>
            			<fmt:message key="jsp.language.${fn:trim(lookup6Form.lookups[row.index].lang)}"/>
            		</legend>        
     <table>
        	<tr>
        		<td>
	...
	<tr>
            <td>
             ...
             <td>
                <form:input path="lookupTitleShort" maxlength="1" size="50"/>
                (<fmt:message key="jsp.general.maxcharacters.args">
                        <fmt:param>1</fmt:param>
                    </fmt:message>)
             </td>
             <form:errors path="lookupTitleShort" cssClass="error" element="td"/>            
        </tr>
		<tr>
		<td>
			<label>
