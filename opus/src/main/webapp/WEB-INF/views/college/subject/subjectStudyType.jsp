...
<div id="wrapper">
    ...
    <spring:bind path="subjectStudyTypeForm.subjectStudyType">
        <c:set var="subjectStudyType" value="${status.value}" scope="page" />
    </spring:bind>

    <spring:bind path="subjectStudyTypeForm.navigationSettings">
        <c:set var="navigationSettings" value="${status.value}" scope="page" />
    </spring:bind>

    <spring:bind path="subjectStudyTypeForm.subject">
    	...
    	<form>
    		<fieldset>
    			<legend>
					...
					&nbsp;&nbsp;<fmt:message key="jsp.general.add" />/<fmt:message key="jsp.general.edit" />&nbsp;<fmt:message key="jsp.general.studytype" /> 
				</legend>
			</fieldset>
		</form>
                    ...
                    <div class="TabbedPanelsContent">
                        <div class="Accordion" id="Accordion1" tabindex="0">
                            <div class="AccordionPanel">
                                <div class="AccordionPanelTab"><fmt:message key="jsp.general.studytypes" /></div>
                                <div class="AccordionPanelContent">

                                <form method="post" name="subjectstudytypedata">

                                    <table>
                                    
                                    	<!-- STUDYTYPE -->
                                    	<tr>
                                            <td class="label"><fmt:message key="jsp.general.studytype" /></td>
                                            <spring:bind path="subjectStudyTypeForm.subjectStudyType.studyTypeCode">
                                            <td class="required">
                                                <select name="${status.expression}">
                                                    <option value="0"><fmt:message key="jsp.selectbox.choose" /></option>
                                                    <c:forEach var="oneStudyType" items="${allStudyTypes}">
	                                                    <c:set var="disabled" value="" scope="page" />
                                                        <c:choose>
                                                            <c:when test="${status.value == oneStudyType.code}">
                                                                <option value="${oneStudyType.code}" selected="selected"><c:out value="${oneStudyType.code}"/></option>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <!-- if studyType is already linked to the subject: don't show the studyType -->
                                                                <c:forEach var="oneSubjectStudyType" items="${subjectStudyTypeForm.allSubjectStudyTypes}">
                                                                    <c:choose>
                                                                        <c:when test="${oneSubjectStudyType.studyTypeCode == oneStudyType.code}">
                                                                ...
                                                                <c:choose>
                                                                    <c:when test="${disabled == ''}">
                                                                        <option value="${oneStudyType.code}"><c:out value="${oneStudyType.description}"/></option>
                                                                    </c:when>
                                                                </c:choose>
                                                            </c:otherwise>
                                                        </c:choose>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                            <select name="subjectStudyType.active">
                                                <c:choose>
                                                    <c:when test="${'Y' == subjectStudyType.active}">
	                                    ...
	                                    <tr><td class="label">&nbsp;</td><td><input type="button" name="submitsubjectstudytypedata" value="<fmt:message key="jsp.button.submit" />" onclick="document.subjectstudytypedata.submit();"/></td></tr>
	                                </table>
