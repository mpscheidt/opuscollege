...
<c:choose>
    <c:when test="${(fn:length(items) lt MAX_DISPLAY_ITEMS) || (showAll == true)}">
        ...
        <table class="tabledata" id="TblData">
            <tr>
                <th style="width:5%"><input type="checkbox" id="checker" checked="checked" onclick="javascript:checkAll('studyGradeIdsCardinalTimeUnits');"/><fmt:message key='jsp.general.all' /></th>
            ...
            <c:forEach var="item" items="${items}">
                <tr>
                    <td align="center">
                       <input type="checkbox" 
                               name="studyGradeIdsCardinalTimeUnits" 
                               checked="checked"
    ...
    <c:otherwise>
        <h2>
            ...
            <a target="_self" href="<c:url value='/ucm/moodleexportstudentstimeunit.view'/>?showAll=true&<%=request.getQueryString() %>">          
