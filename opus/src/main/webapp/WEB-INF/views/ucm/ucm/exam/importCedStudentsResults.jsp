	...
	<tr>
		...
		<td class="required">
			...
			<c:forEach var="fileModel" items="${resultsForm.fileModels}">
				...
				<c:choose>
				<c:when test="${fileModel.value != 'CED_PAUTA'}">
 					<form:radiobutton path="fileModel" value="${fileModel}" 
 						id="${fileModel.value}Radio" label="${fileModel.value}"
 						disabled="true"
 					/>
 					</c:when>
 					<c:otherwise>
			...
			<%--
			<label for="pautaRadio">Pauta</label>
			<input type="radio" name="fileModel" id="pautaRadio"/>
			
			<label for="machineRadio" >Maquina</label>
			<input type="radio" name="fileModel" id="machineRadio"/>
			 --%>
		</td>
	</tr>
	<tr>
