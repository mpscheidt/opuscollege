...
<body>
	...
	<div id="tabwrapper">
		...
		<div id="tabcontent">
			<form:form method="post" action="multiplereallocation.view"
				commandName="studentsForm">
				<input type="hidden" value="<c:out value="${param.preferredLanguage}" />" name="preferredLanguage" />
				<fieldset>
					<legend><fmt:message key="jsp.accommodation.multipleReallocationOverview" /></legend>
					<table>
						<tr>
						...
						<tr>
							...
							<td><form:select path="studyGradeTypeId"
									...
									<c:forEach items="${allStudyGradeTypes}" var="studyGradeType">
										<form:option value="${studyGradeType.id }"><c:out value='${studyGradeType.gradeTypeDescription} - ${studyGradeType.studyDescription } - ${studyGradeType.studyDescription } (${mapAcademicYear[studyGradeType.currentAcademicYearId]})' /></form:option>
									</c:forEach>
								</form:select></td>
						</tr>
						<tr>
							...
							<td>&nbsp;</td>
							<td><fmt:message key="jsp.accommodation.cardinalTimeUnitNumber" /></td>
			...
			<c:choose>
				<c:when test="${not empty applicants}">
					<form action="multiplereallocation.view" method="post">
						...
						<table id="tblListStudentAccommodation" class="tabledata">
							<tr>
								...
								<th><fmt:message key="jsp.accommodation.otherNames" /></th>
								...
								<th><input type="submit" value="<fmt:message key="${jsp.accommodation.allocate}" />" name="allocate" /></th>
							</tr>
							<c:forEach items="${applicants }" var="studentAccommodation">
								<tr>
								   <td><input type="checkbox" name="studentAccommodationId" value=<c:out value='"${studentAccommodation.id}' />" checked="checked" /></td>
									...
									<td><c:out value='${studentAccommodation.student.firstnamesAlias }' /></td>
									...
									<td title=""><c:out value='${studentAccommodation.academicYear.description}' /></td>
