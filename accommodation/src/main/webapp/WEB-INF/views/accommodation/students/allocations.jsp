	...
	<div id="tabwrapper">
		...
		<div id="tabcontent">
			<form:form method="post" 
				...
				<fieldset>
					<legend>
						<fmt:message key="jsp.accommodation.allocationsOverview" />
					</legend>

					<%@ include file="../../includes/applicantsFiltersTable.jsp" %>
					<%@ include file="../../includes/allocationsFiltersTable.jsp" %>
		...
		<c:choose>
				<c:when test="${not empty allocationsForm.allocations}">
					<c:set var="allEntities" value="${allocationsForm.allocations}" scope="page" />
					...
					<table id="tblListStudentAccommodation" class="tabledata">
						...
						<c:forEach items="${allocationsForm.allocations }" var="studentAccommodation" varStatus="rowNumber">
							<tr>
								...
								<sec:authorize access="hasRole('ALLOCATE_ROOM')">  
									<td><c:choose>
											<c:when test="${studentAccommodation.allocated=='Y'}">
												...
												<a href="<c:url value='allocateResource.view'/>?<c:out value='newForm=true&studentAccommodationId=${studentAccommodation.id}&allocate=true&preferredLanguage=${param.preferredLanguage}' />">Resource Allocation</a>
											</c:when>
											<c:otherwise>
												<a
													href="<c:url value='roomallocation.view'/>?<c:out value='newForm=true&studentAccommodationId=${studentAccommodation.id}&preferredLanguage=${param.preferredLanguage}' />">Allocate</a>
											</c:otherwise>
										</c:choose></td>
								</sec:authorize>
							</tr>
