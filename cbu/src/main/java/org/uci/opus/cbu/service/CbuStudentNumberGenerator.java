 ...
 *
 */
public class CbuStudentNumberGenerator implements StudentNumberGeneratorInterface {

    @Autowired private CbuManagerInterface cbuManager;

    private static Logger log = LoggerFactory.getLogger(CbuStudentNumberGenerator.class);
    ...
    public String createUniqueStudentCode(String key, int organizationalUnitId, Student student) {
        ...
        return studentCode;
//        if (organizationalUnitId == 0) return null;
        
//        int appNumber = student.getStudyPlans().get(0).getApplicationNumber();
//        if (appNumber == 0) {
//            throw new RuntimeException("Application number is empty -> cannot create student number");
//        }
        
        // only use the last 5 digits, even if application number is longer (do a modulo operation)
//        appNumber = appNumber % 100000;
                
        // if appNumber shorter than 5 digits, fill up with leading zeroes 
//        String appNumber5 = String.valueOf(appNumber);
//        appNumber5 = StringUtil.prefixChars(appNumber5, 5, '0');
    ...
    private String createApplicationNumber() {
        ...
        Integer nextApplicationNumber = cbuManager.findNextApplicationNumber();
