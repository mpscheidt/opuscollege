 ...
 *
 */
public class BankDeleteController extends AbstractController {
    
    private static Logger log = LoggerFactory.getLogger(BankDeleteController.class);
    ...
    protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // delete chosen scholarship:
        if (bankId != 0) {
            // bank cannot be deleted if scholarship students are present
            List < ScholarshipStudent > scholarshipStudents = null;
            scholarshipStudents = scholarshipManager.findScholarshipStudentsForBank(bankId);
            if (scholarshipStudents.size() != 0) {

              Locale currentLoc = RequestContextUtils.getLocale(request);

                // show error
               showBanksError = messageSource.getMessage(
                       "jsp.error.general.delete.bank", null, currentLoc);
            } else {
              scholarshipManager.deleteBank(bankId);
            }
          
        }
        ModelAndView mav = new ModelAndView();
        mav.setViewName("redirect:/scholarship/banks.view?currentPageNumber=" + currentPageNumber);
        mav.getModelMap().put("showBanksError", showBanksError);
