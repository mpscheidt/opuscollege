...
<c:choose>
    <c:when test="${(fn:length(items) lt MAX_DISPLAY_ITEMS) || (showAll == true)}">
        ...
        <div id="abc">&nbsp;</div>
          ...
          <tr> 
            <th><input type="checkbox" name="checker" id="checker" checked="checked" onclick="javascript:checkAll('studentsIds');"/><fmt:message key='jsp.general.create.report' /></th>
            ...
            <c:forEach var="student" items="${items}">
                    <tr>
                        <td align="center"><input type="checkbox" name="studentsIds" checked="checked"  value="${student.studentId}"></td>
                         <td>${student.studentCode}</td>                        
                        <td>${student.firstNamesFull}</td>
                        <td>${student.surnameFull}</td>
                        <td>${student.studyDescription}</td>
                       <td>${student.studyPlanDescription}</td>
    ...
    <c:otherwise>
        ...
        <a target="_self" href="<c:url value='/ucm/exportstudents.view'/>?<%=request.getQueryString() %>&showAll=true">
