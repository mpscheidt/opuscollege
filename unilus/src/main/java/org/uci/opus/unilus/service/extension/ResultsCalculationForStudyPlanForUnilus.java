 ...
 * License.
 * 
 * The Original Code is Opus-College unilus module code.
 * 
 ...
 * ***** END LICENSE BLOCK *****
 ******************************************************************************/
package org.uci.opus.unilus.service.extension;
...
/* 
 * Calculation of studyplan result:
 * fetch senior subjects (year 3, 4 (and 5)) for calculation of studyPlanResult
 * and take optional thesis into account
 */

public class ResultsCalculationForStudyPlanForUnilus 
    implements ResultsCalculations {

    private static Logger log = Logger.getLogger(ResultsCalculationForStudyPlanForUnilus.class);
    ...
    public StudyPlanResult calculateResultsForStudyPlan(final Study study, 
        ...
        HashMap endGradeMap = new HashMap();
        ...
        if (allSubjectsForStudyPlan != null 
        	...
        	if (log.isDebugEnabled()) {
                log.debug("allSubjectsForStudyPlan.size() = " 
            ...
            subjectResultsForStudyPlan = resultManager.findCalculatableSubjectResultsForStudyPlan(
                    allStudyPlanDetailsForStudyPlan, 
                    allSubjectsForStudyPlan, studyPlan, true);
            ...
            if (subjectResultsForStudyPlan == null || subjectResultsForStudyPlan.size() == 0) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("calculatable subjectResults.size() " 
                            + subjectResultsForStudyPlan.size());
                }
            }
            calculatableSubjects = resultManager.findCalculatableSubjectsForStudyPlan(
                    preferredLanguage, studyPlan, "allSeniorYears");
            
            
        } else {
            if (log.isDebugEnabled()) {
                log.debug("allSubjectsForStudyPlan.size() IS NULL " +
        ...
        if ("".equals(errorMsg) && calculatableSubjects == null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("calculatableSubjects.size() = " +
...
//                                        && saveMaximumEndGradeStudyGradeType != maximumEndGradeStudyGradeType) {
                        ...
                        if (!"".equals(errorMsg)) {
                            if (log.isDebugEnabled()) {
                                log.debug("error different scales: " + errorMsg);
                            }
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("no subjectresults available");
                    }
                }
            }
        }
        ...
        if ("".equals(errorMsg)) {
            if (log.isDebugEnabled()) {
                log.debug("start calculation for all calculatableSubjects.size()"
                        + calculatableSubjects.size());
            }
            for (int h = 0; h < calculatableSubjects.size(); h++) {
...
//                                        subjectResultMarkDouble = Double.parseDouble(subjectResultMark);
                                ...
                                if (log.isDebugEnabled()) {
                                    log.debug(subject.getSubjectDescription() + ", subjectResultMarkDouble =  " + subjectResultMarkDouble);
                                }
...
//                                if (subjectResultMarkDouble > maxSubjectResultMarkDouble) {
                        ...
                        if (subjectResultForSubjectFound == false) {
                            ...
                            examMark = "0.0";
                            if (log.isDebugEnabled()) {
                                log.debug("subjectResultForSubjectFound == false");
                            }
                            break;
                        }
                ...
                if ("".equals(errorMsg)) { 
                    if (log.isDebugEnabled()) {
                        log.debug(subject.getSubjectDescription() + " - maxSubjectResultMarkDouble:" + maxSubjectResultMarkDouble);
                    }
//                    examMarkDouble = examMarkDouble + (creditAmount * maxSubjectResultMarkDouble);
            ...
            if (log.isDebugEnabled()) {
                log.debug("after examMark calculations: examMarkDouble = " 
...
//                            thesisMarkDouble = (double) thesisMarkInt;
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("step 1 calculation thesisMarkDouble = " 
...
//                    thesisMarkDouble = thesisMarkDouble * thesisCreditAmount;
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("step 2 calculation thesisMarkDouble = " 
                                + thesisMarkDouble + ", thesisCreditAmount = " + thesisCreditAmount);
                    }
                }
            }
...
//                    totalCreditAmount = totalCreditAmount + thesisCreditAmount;
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("before end-calculation thesisMark = " 
                ...
                if (log.isDebugEnabled()) {
                    log.debug("BEFORE end-calculation totalCreditAmount = " + totalCreditAmount
                            + ", examMarkDouble = " + examMarkDouble);
                }
...
//                examMarkDouble = su.formatDoubleMark(examMarkDouble);
                ...
                if (log.isDebugEnabled()) {
                    log.debug("AFTER end-calculation totalExamMarkDouble = " 
                            + examMarkDouble);
                }
...
//                    examMarkInt = (int) Math.round(examMarkDouble);
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("examMark is string:" + examMark);
                    }
                } else {
                    if (examMarkInt != 0) {
...
//                        examMark = "" + su.formatDoubleMark(examMarkDouble);
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("examMark is int / double:" + examMark);
                    }
                }
            }
        }
        if (!"".equals(errorMsg) ) {
            if (log.isDebugEnabled()) {
                log.debug("after calculation errMsg is not empty: "
                        + errorMsg);
            }
            // return errormessage:
