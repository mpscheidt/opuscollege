 ...
 *
 */
public class MoveToNextCardinalUnitSurplusPaymentsController extends AbstractController {

    private static Logger log = LoggerFactory.getLogger(MoveToNextCardinalUnitSurplusPaymentsController.class);
     ...
     */
    public MoveToNextCardinalUnitSurplusPaymentsController() {
        super();
    }
...
//        List < Payment > rejectedPaymentsByInstallmentNumber = null;
        ...
        if(feeType==1){
        	...
        	allPaymentsToPayForNextCardinalUnit=paymentUtil.createSubjectBlockStudyGradeTypesSurplusPaymentsForNextCardinalUnit(studentFeesToLoad, allPaymentsForStudent, allStudentBalances, studyGradeTypeId, iUseOfPartTimeStudyGradeTypes);
        }
        else if(feeType==2){
        	studentFeesToLoad=feeManager.findStudentFeesForSubjectStudyGradeTypes(studentId);
         	allPaymentsToPayForNextCardinalUnit=paymentUtil.createSubjectStudyGradeTypesSurplusPaymentsForNextCardinalUnit(studentFeesToLoad, allPaymentsForStudent, allStudentBalances, studyGradeTypeId, iUseOfPartTimeStudyGradeTypes);        	
        }
        else if(feeType==3){
        	studentFeesToLoad=feeManager.findStudentFeesForStudyGradeTypes(studentId);
         	allPaymentsToPayForNextCardinalUnit=paymentUtil.createStudyGradeTypesSurplusPaymentsForNextCardinalUnit(studentFeesToLoad, allPaymentsForStudent, allStudentBalances, studyGradeTypeId, iUseOfPartTimeStudyGradeTypes);
        }
        else if(feeType==4){
        	studentFeesToLoad=feeManager.findStudentFeesForAcademicYears(studentId);
            allPaymentsToPayForNextCardinalUnit=PaymentUtil.createAcademicYearSurplusPaymentsForNextCardinalUnit(studentFeesToLoad,allPaymentsForStudent,allStudentBalances,allAcademicYears,academicYearId);
        }
