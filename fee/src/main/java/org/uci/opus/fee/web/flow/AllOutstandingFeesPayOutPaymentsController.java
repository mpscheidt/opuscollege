 ...
 *
 */
public class AllOutstandingFeesPayOutPaymentsController extends AbstractController {

    private static Logger log = LoggerFactory.getLogger(AllOutstandingFeesPayOutPaymentsController.class);
     ...
     */
    public AllOutstandingFeesPayOutPaymentsController() {
        super();
    }
    ...
    protected ModelAndView handleRequestInternal(
        ...
        List < Payment > paymentsToPayAllForStudeyGradeType = null;
        List < Payment > rejectedPaymentsToPayAllForStudeyGradeType = null;
        ...
        if(feeType==1){
            ...
            allPaymentsToPayForStudyGradeType=PaymentUtil.createPaymentsToPayAllForStudyGradeType(studentFeesToLoad,allPaymentsForStudent,allStudentBalances,studyGradeTypeId);
        }
        else if(feeType==2){
            ...
            allPaymentsToPayForStudyGradeType=PaymentUtil.createPaymentsToPayAllForAcademicYear(studentFeesToLoad,allPaymentsForStudent,allStudentBalances,academicYearId);
        }       
        

        paymentsToPayAllForStudeyGradeType=allPaymentsToPayForStudyGradeType.get("approevedPayments");
        rejectedPaymentsToPayAllForStudeyGradeType=allPaymentsToPayForStudyGradeType.get("rejectedPayments");

        for(Payment payment:paymentsToPayAllForStudeyGradeType){
        	paymentManager.addPayment(payment, opusMethods.getPreferredLanguage(request));
        }
        for(Payment payment:rejectedPaymentsToPayAllForStudeyGradeType){
