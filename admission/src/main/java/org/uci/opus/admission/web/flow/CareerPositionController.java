...
import org.uci.opus.college.validator.CareerPositionValidator;
    ...
    @Controller
    @RequestMapping("/careerposition_admission.view")
    @SessionAttributes({"requestAdmissionForm"})
    public class CareerPositionController {
        
        private static Logger log = LoggerFactory.getLogger(CareerPositionController.class);
        private String formView;
        @Autowired private CareerPositionValidator careerPositionValidator;
        @Autowired private MessageSource messageSource;

        public CareerPositionController() {
            super();
            this.formView = "admission/careerPosition";
        }
        ...
        public String processSubmit(
            ...
            if (log.isDebugEnabled()) {
                log.debug("CareerPositionController.processSubmit entered...");  
            }

            careerPositionValidator.onBindAndValidate(request, requestAdmissionForm, result);
            if (result.hasErrors()) {
                ...
                List < CareerPosition > allCareerPositions = requestAdmissionForm
                                                                    .getAllCareerPositions();
                allCareerPositions.add(requestAdmissionForm.getNewCareerPosition());
                
                requestAdmissionForm.setAllCareerPositions(allCareerPositions);
                requestAdmissionForm.setNewCareerPosition(null);
            }
